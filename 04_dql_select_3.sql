...
# 그룹 내 하위 그룹 구성 가능
# category_code로 1차 그룹화 후 각 그룹에서 orderable_status가 같은 행 끼리 2차 그룹화
select
    category_code,
    orderable_status,
...
# 메뉴 테이블에서 주문이 가능한 메뉴 중 카테고리 코드가 4,10인 메뉴의 카테고리별 개수 조회
select
    c.category_name,
    count(*)
from
...
# 메뉴 테이블에서 카테고리 별 메뉴 개수가 2개 이상인 카테고리의
# 카테고리 번호, 개수 출력
select
    category_code,
    count(*)
...
# 카테고리 테이블에서
# 부모 카테고리(ref_category_code_ 별로 개수 3개 이상인
# 부모 카테고리 번호, 개수 조회
# 부모 카테고리 오름 차순으로 조회
select
    ref_category_code,
