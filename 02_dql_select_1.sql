...
select
    menu_name as 메뉴명,
    menu_price as "메뉴 가격", # 쌍따옴표를 이용해서 띄어쓰기된 컬럼명 묶기
    menu_price * 0.1 부가세,
    menu_price + (menu_price * 0.1) "부가세 포함 가격" # as 생략
from
    tbl_menu;
...
# - 기준으로 삼은 컬럼이 여러 개 존재하면 그룹화된 정렬이 수행된다.
# 메뉴 테이블에서 메뉴명, 카테고리 코드, 가격을 조회
# 단, 카테고리 코드 오름차순, 가격 내림차순으로 조회
# 1) 카테고리 코드로 오름차순 정렬을 먼저 수행
# 2) 같은 카테고리 코드를 지닌 행들끼리 붙어지게 됨
...
# 메뉴 테이블에서 가격이 13000원 이상인 메뉴의 메뉴코드, 메뉴명, 가격 조회
# 가격 내림차순 정렬
select
    menu_code,
    menu_name,
...
# 메뉴의 메뉴명, 가격, 카테고리 코드를 가격 내림차순으로 조회
select
    menu_name,
    menu_price,
    category_code
...
#     10000
#     and
#     20000 => 10000 ~ 20000 사이
order by
    menu_price desc;

# 반대 케이스 -> 10,000원 미만 또는 20,000원 초과
select
    menu_name,
    menu_price,
    category_code
...
# 메뉴 테이블에서 카테고리 코드가 4,6,10 인 메뉴의
# 메뉴명, 카테고리 코드 조회
# 정렬 : 카테고리 코드 오름차순
select
    menu_name,
...
#     ref_category_code is null; # ref_category_code 컬럼에 값이 비어있으면 True
    ref_category_code is not null; # 값이 있는 애들만 나옴
