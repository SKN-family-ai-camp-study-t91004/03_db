...
# UTF-8 : 아스키코드 문자는 1byte, 나머지는 3byte 표시 (mysql 사용)
...
# INSTR(기준문자열, 부분(검색)문자열): 기준 문자열에서 부분 문자열의 시작 위치 반환
SELECT
    instr('사과딸기바나나', '딸기'); # 3
...
select
    ...
    truncate(1234.56, -2); # 10의 자리 버림 -> 1200
...
# - date에서 해당하는 단위 추출 -> 숫자 형태
# - 단위 : year, quarter, month, week, day, hour, minute, second, microsecond
